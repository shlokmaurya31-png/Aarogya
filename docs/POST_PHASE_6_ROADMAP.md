...
**P0 — RESOLVED in Phase 6.6 (see `docs/PHASE_6_6_INTEGRITY.md` for full detail)**

1. ~~**Medication-to-inventory link table has no creation path.**~~ **RESOLVED.** A real, authenticated, audited write path now exists: `POST /api/hospital/inventory/medication-links`, gated on `inventory:item:manage`, validating item existence/active/category/facility before creating the link. Seed data extended to cover all 5 general-loop drug names.
2. ~~**Invoice payment-allocation over-allocation race.**~~ **RESOLVED.** `Invoice.allocatedMinor` is now a guarded running total, atomically maintained by a threshold-guarded `UPDATE` in the same transaction as the pre-existing `Payment`-side CAS. Verified against genuine concurrent Postgres transactions (5/5 required race cases pass).
3. ~~**Medication administration double-recording race.**~~ **RESOLVED.** `administerMedication` now uses the same guarded-`updateMany`+count-check idiom as every sibling CAS function in the codebase. Verified against genuine concurrent Postgres transactions.
4. ~~**Inventory/Diagnostics live-render discrepancy.**~~ Re-verified via a full clean rebuild + authenticated HTTP requests against the actually-rendered HTML — did not reproduce across 6 tested roles. No code defect was found; treated as a resolved transient dev-environment artifact, not a persistent bug. Full interactive browser verification remains pending (tooling unavailable this session).
...
11. ~~The Inventory/Diagnostics navigation live-render discrepancy...~~ Re-verified in Phase 6.6, did not reproduce — see item 4 in the resolved-P0 list above.
21. **New, found during Phase 6.6's own investigation of item 3 above**: the same read-check-then-plain-`update()` TOCTOU shape is latent throughout `medicationLifecycle.ts`'s shared `transition()` helper (used by `verifyMedicationOrder`/`rejectMedicationOrder`/`holdMedicationOrder`/`dispenseMedication`/`cancelMedicationOrder`) and in `appointment.ts`'s `cancelAppointment`/`markNoShow`/`checkInAppointment` — none of these use the guarded-`updateMany` idiom either. Only `administerMedication` was in Phase 6.6's named scope (direct double-dosing risk); the rest should be closed before a domain that leans harder on high-frequency order-status transitions.
...
**Update (Phase 6.6, `docs/PHASE_6_6_INTEGRITY.md`)**: all three P0 items and the Inventory/Diagnostics render discrepancy are now resolved and verified against genuine Postgres concurrency (93/93 assertions). The verdict above is still **NO** — Nursing assessment/charting (item 7 in the P1 list) is untouched and remains the explicit prerequisite for ICU flowsheets, and this was never in Phase 6.6's scope. Do not treat P0 closure as ICU readiness on its own.

**Update (Phase 6.7, `docs/PHASE_6_7_NURSING.md`)**: Nursing assessment/charting is now built — a real `NursingAssessment` lifecycle (DRAFT -> COMPLETED -> SIGNED -> SUPERSEDED), a flowsheet composition layer, real vitals/I-O UI, a structured handoff workflow with an acknowledge inbox, and Task/ClinicalNote/ClinicalHandoff/NursingAssignment concurrency hardening matching Phase 6.6's medication-administration fix, all verified against genuine Postgres concurrency (67/67 assertions, including the Phase 6.6 medication regression). The verdict above still does **not** automatically become YES — Nursing being complete removes one named ICU prerequisite, but ICU also needs its own additive schema design (section 6: Equipment/EquipmentAssignment, coded GCS/sedation, VentilatorSetting, DeviceLine, Infusion), which Phase 6.7 deliberately did not touch (out of Nursing Core's scope). See `docs/PHASE_6_7_NURSING.md` §12 for Phase 6.7's own remaining gaps before treating any phase after it as ICU-ready.
