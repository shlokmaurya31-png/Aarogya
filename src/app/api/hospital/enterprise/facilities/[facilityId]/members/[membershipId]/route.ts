...
import { setFacilityMembershipScope, removeFacilityMembership } from "@/lib/enterprise/memberships";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ membershipId: string }> }) {
  return withApiErrors(async () => {
    ...
    return { membership: await setFacilityMembershipScope(m, membershipId, parsed.data) };
  });
}
