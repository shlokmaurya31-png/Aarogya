...
export async function GET(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    const orders = await prisma.labOrder.findMany({
      ...
      include: { patient: true, encounter: true, result: true, orderedBy: { include: { user: true } } },
...
export async function POST(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    const { encounterId, patientId, testName, category, priority } = body ?? {};
    ...
    const order = await prisma.labOrder.create({
      data: { encounterId, patientId, testName, category, priority: priority ?? "ROUTINE", orderedByStaffId: staff.id },
    });
    ...
    await recordAuditEvent("hospital.lab.ordered", session.userId, { orderId: order.id });
    return { order };
  });
}
